    ...
    "When we set up two apps": {
        ...
        "and we register one user on each": {
            topic: function() {
                ...
                Step(
                    function() {
                        newCredentials("collector", "give*me*that", "social.localhost", 80, this.parallel());
                        newCredentials("friedlander", "shadowed*1", "photo.localhost", 80, this.parallel());
                    },
                    callback
                );
            },
            ...
            "and one user follows the other": {
                topic: function(cred1, cred2) {
                    var url = "http://social.localhost/api/user/collector/feed",
                        act = {
                            verb: "follow",
                            object: {
                ...
                "and we wait a few seconds for delivery": {
                    ...
                    "and the second user posts an image publicly": {
                        topic: function(act, cred1, cred2) {
                            var url = "http://photo.localhost/api/user/friedlander/feed",
                                callback = this.callback,
                                post = {
                                    verb: "post",
                                    to: [{objectType: "collection", id: "http://activityschema.org/collection/public"}],
                                    object: {
                                        objectType: "image",
